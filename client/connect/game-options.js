...
const GameOptions = props => { 
    ...
    const {setStart, startNewGame, difficulty, setDifficulty, cursor, setPlayer, gameType, setCursor} = props
  ...
  return(
      <div className='flex-col'>
        ...
        {gameType === "vsLocal" &&
            <div className="vs-local-title">Who Goes First?</div>
        }
        <div id='players'>
            <a onClick={()=>{handleClick('r')}}><div className={cursor == 'r' ? 'r options-piece'+' selected': 'r options-piece'}></div></a>
            <a onClick={()=>{handleClick('y')}}><div className={cursor == 'y' ? 'y options-piece'+' selected': 'y options-piece'}></div></a>
        </div>
        {gameType === 'vsAI'&&
        <div className='flex-col'>
